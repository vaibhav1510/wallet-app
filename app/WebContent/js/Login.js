...
function genOrderSummary(email, password) {
	...
	//  			  
	// // url = "http://localhost:8080/app/index.html" ;//+ "?email=" + emailId
	...
	// Console.log("ERRORR");
	// var respJSON =
	// ajaxResponseHandlers["defhandler"].parse(jqXHR.responseText);
	// if (_.has(respJSON, "errors")) {
	// ajaxResponseHandlers["formhandler"].error(jqXHR, textStatus,
	// errorThrown);
	// }
	// }
	// });
...
$('#Submit-Login').click(function() {
	...
	// var encryptedPassword = CryptoJS.AES.encrypt(password, "My Secret
	// Passphrase");
	 ...
	 * 
	 * var Initprojectinfo = new SysSynSimAppApi.Projectdetailinfo152(); //
	 * Construct a model instance. Initprojectinfo.simapp_id = gSimappId;
	 * 
	 * var PreProcessorWorkerSvc = new SysSynSimAppApi.PreProcessorWorkerApi(); //
	 * Allocate the API class we're going to use. var yyy =
	 * PreProcessorWorkerSvc.getProjectDetails152(Initprojectinfo,
	 * callbackStudies); // Invoke the PreProcessorWorker service });
	 */
