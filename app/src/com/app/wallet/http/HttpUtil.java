...
public class HttpUtil{
    ...
    public Response post(String url, String content, Map<String, String> extraHeaders, boolean throwIfEmptyResp) throws IOException {
    ...
    private Response doFormSubmit(String url, Method m, String queryStr, Map<String, String> extraHeaders, boolean throwIfEmptyResp) throws IOException {
    ...
    private static String getAuthValue(HttpConfig config){
        switch (config.authScheme) {
            case BASIC:
                return "Basic " + Base64.encodeBase64String((config.username + ":"
                        + (config.password != null ? config.password : "")).getBytes())
